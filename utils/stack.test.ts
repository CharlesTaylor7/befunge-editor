...
describe('Stack', () => {
  ...
  describe('#isEmpty', () => {
    ...
    test('a non empty stack is not empty', () => {
      expect(Stack.isEmpty(Stack.push(1, Stack.empty)))
        .toBe(false)
    })
  })
  describe('#fromArray', () => {
  ...
  describe('#pop', () => {
    test('is curried', () => {
      expect(Stack.pop(1)(Stack.fromArray([1])))
        .toEqual(Stack.pop(1, Stack.fromArray([1])))
    })
    test('can get top of stack', () => {
      expect(Stack.pop(1, Stack.fromArray([1, 2, 3])))
        .toEqual([1, Stack.fromArray([2, 3])])
    })
    test('can get many from top of stack', () => {
      const stack = Stack.fromArray([1,2,3,4])
      expect(Stack.pop(2, stack))
        .toEqual([1, 2, Stack.fromArray([3, 4])])
    })
    test('popping zero returns singleton array', () => {
      const stack = Stack.fromArray([3, 4])
      expect(Stack.pop(0, stack))
        .toEqual([stack])
    })
    test('popping more than stack depth yields 0 as placeholder', () => {
      expect(Stack.pop(2, Stack.fromArray([3])))
        .toEqual([3, 0, Stack.empty])
    })
  })
  describe('#fromString', () => {
  ...
  describe('is iterable', () => {
    ...
    test('iterate non empty stack', () => {
      expect(Array.from(Stack.fromArray([1, 2])))
        .toEqual([1, 2])
    })
  })
  describe('is immutable', () => {
    test('empty stack is immutable', () => {
      expect(() => (Stack.empty as any).foo = "Foo")
        .toThrow()
    })
    test('non empty stack is immutable', () => {
      expect(() => (Stack.fromArray([3, 5]) as any).bar = "Bar")
        .toThrow()
    })
  })
  describe('can be destructured', () => {
    test('non empty stack yields head & tail', () => {
      const { head, tail } = Stack.fromArray([2, 0])
      expect(head).toBe(2)
      expect(tail).toEqual(Stack.fromArray([0]))
    })
